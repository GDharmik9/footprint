...
import { registerUser, getUserDetails, updateUserProfile, getUserInsights } from './controllers/userController.js';
...
app.patch('/api/users/:id', authenticateToken, updateUserProfile);
...
app.post('/api/carbon-events', authenticateToken, createEvent);

// Challenge routes
app.get('/api/challenges/:userId', authenticateToken, getChallenges);
app.post('/api/challenges/progress', authenticateToken, updateChallengeProgress);

// Sponsor/Reward/League routes
app.post('/api/sponsors/redeem', authenticateToken, redeemVoucher);
...
app.post('/api/integrations/arcadia/callback', authenticateToken, handleArcadiaCallback);
